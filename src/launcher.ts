// Launches on-device browsers through HDC and wires them into
 ...
 */
export function resolveOhosAa(): string | undefined {
...
function execAsync(command: string, args: string[], timeoutMs = 30000): Promise<{ stdout: string; stderr: string }> {
...
export function resolveLaunchConfig(options: { channel?: string; harmonyBundleName?: string; harmonyDebugPort?: number }): LaunchConfig {
...
async function findDevtoolsSocket(hdc: HdcBackend, timeoutMs = 30000): Promise<string> {
...
async function startBrowserViaHdc(hdc: HdcBackend, config: LaunchConfig): Promise<string> {
...
function patchBrowserClose(browser: any, hdc: HdcBackend, config: LaunchConfig): void {
 ...
 */
export async function launchViaHdc(chromium: any, progress: any, options: any): Promise<any> {
