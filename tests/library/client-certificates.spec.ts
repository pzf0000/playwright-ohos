  ...
  startCCServer: async ({ asset }, use) => {
    ...
    await use(async options => {
      server = (options?.http2 ? createHttp2Server : createHttpsServer)({
        ...
        parts.push({ key: 'servername', value: tlsSocket.servername as string });
