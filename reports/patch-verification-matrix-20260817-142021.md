...
- 报告生成时间：2026/8/17 14:20:21（本地时间）
...
|---|---|---|---|---|---|---|
| patch-6-reuse-page | ArkWeb 复用页面 | chrome | 1 | 0 | 0 | 华为：不应用时 context.newPage 失败。原 grep 命中的是 browser.newPage（华为有意拦截），已更换为 context.newPage 路径。 |
| patch-6-reuse-page | ArkWeb 复用页面 | huaweiBrowser | 1 | 0 | 0 | 华为：不应用时 context.newPage 失败。原 grep 命中的是 browser.newPage（华为有意拦截），已更换为 context.newPage 路径。 |
| patch-6b-close-page | ArkWeb 页面软关闭 | chrome | 5 | 0 | 0 | 华为：不应用时关闭页面破坏会话。 |
| patch-6b-close-page | ArkWeb 页面软关闭 | huaweiBrowser | 3 | 2 | 0 | 华为：不应用时关闭页面破坏会话。 |
