...
${indent}if (!localCacheDir)`,
    ...
    },
    {
        ...
        replace: () => `if (!this._isArkWeb)
          assert(targetInfo.browserContextId, "targetInfo: " + JSON.stringify(targetInfo, null, 2)); ${marker('patch-2a-context-assert')}`,
    },
    {
...
export function resolveCoreBundlePath(): string {
...
export function applyPatches(coreBundlePath: string): PatchResult[] {
...
export function checkSyntax(coreBundlePath: string): boolean {
