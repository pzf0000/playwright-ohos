...
import { createSkipTestPredicate } from './bidi-expectation-stub';
...
import type { Log } from './ohos-utils';
import type { TestInfo } from '@playwright/test';

import { removeFolders, hostPlatform } from './ohos-utils';
...
// Optional per-test browser restart: a fresh device browser is launched
// before every test (the HDC launcher force-stops the device instance at
// launch) and the worker `browser` fixture forwards to it through a proxy,
// so tests that use `browser` directly also see the live instance.
let ohosCurrentBrowser: any = null;
const ohosBrowserProxy = new Proxy({}, {
  get(_target: any, prop: string | symbol) {
    if (prop === 'then')
      return undefined;
    if (!ohosCurrentBrowser)
      throw new Error('browser fixture used before the test browser was launched');
    const value = ohosCurrentBrowser[prop];
    return typeof value === 'function' ? value.bind(ohosCurrentBrowser) : value;
  },
});
const exportedTest = process.env.PW_OHOS_BROWSER_RESTART === 'per-test'
  ? test.extend<BrowserTestTestFixtures, BrowserTestWorkerFixtures>({
      browser: [async ({}, use) => {
        await use(ohosBrowserProxy);
        if (ohosCurrentBrowser) {
          await ohosCurrentBrowser.close().catch(() => {});
          ohosCurrentBrowser = null;
        }
      }, { scope: 'worker', timeout: 0 } as any],
      _ohosRestartBrowser: [async ({ playwright, browserName, launchOptions, channel }, use) => {
        ohosCurrentBrowser = await playwright[browserName].launch({ ...launchOptions, channel });
        try {
          await use();
        } finally {
          if (ohosCurrentBrowser) {
      ...
      }, { scope: 'test', auto: true, timeout: 0 } as any],
    } as any)
  : test;

export const playwrightTest = exportedTest;
export const browserTest = exportedTest;
export const contextTest = exportedTest;
